...
#%% number of fins plot and analysis
...
plt.xlabel('Length of fin')
plt.ylabel('Temerature of processor')
# ax2.legend()

print(f'The distribution apears to plataue and extrapolating the fit, the data sugests that with an infinite array of processors, the temperature could at best get to {round(b*293-273)} degrees C')

#%% Join length of fin and number of fins data
...
#%% comparing change in overall surface area for fin length and number of fins
...
#%% lenfth of fins with forced convection
...
#%% forced convection increase in mm analysis
...
#%% Final analysis for minimum dimensions
...
def plot_the_temp_5(path, n, starting, ax ,col= 'r'):
    ...
    ax.plot(np.append(T_a,T_b), [lin(i,a,b) for i in np.append(T_a,T_b)], alpha=0.2, color=col)
    ax.plot([-b/a],[0], 'o', color=col)
    print(f'Converge point for {n} fins is {-b/a}')
    n=len(T_a)
    print(f'difference from below to mid = {(-b/a - np.array(T_b)[-1])}')
    print(np.array(T_b)[-1])
    print(f'difference from above to mid = {(-b/a - np.array(T_a)[-1])}\n')
...
(a1, b1), cov = curve_fit(inv_lin, mms, np.array(T_forced))
plt.plot(mms, T_forced, 'o-', label='data: forced convection number of fins', color='black')
plt.plot(mms, [inv_lin(i, a1, b1) for i in mms], '--', label='f$(x^{-1})$ fit', color='purple', lw=4.2)
errors = cov.copy()
plt.xlabel('Temperature K')
plt.ylabel('Total perimiter off the system [mm]')
plt.legend()
plt.show()
...
print('The error on this value is:', 1/((operational_temp-b1)/a1)*np.sqrt(errors[0,0]/a1**2 + errors[1,1]/b1**2))
...
path = 'whole_system_FDO4/fin_length_test_forced'
df = pd.read_csv(f'{path}/data_s6_fin_length_39')
T_b, T_a = df['T_pr_above'][starting:]*293, df['T_pr_below'][starting:]*293
D_b, D_a = df['diff_above'][starting:], df['diff_bellow'][starting:]

(A1,B1), cov = curve_fit(lin, np.append(T_b,T_a), np.append(D_b,D_a))
print(f'\nFor the 39mm run with 20 fins, the temperature converged to is {-B1/A1 -273}\nAnd after considering systematic error from scaling, the value is {-B1/(A1*scaling_factor) -273}')
print('The error on this value is:', round((-B1/A1 -273)*np.sqrt(cov[0,0]/A1**2 + cov[1,1]/B1**2)*7), 'Degress C\n')
# print(f'error on perimiter needed = {}')
df = trim(np.matrix(pd.read_csv(f'{path}/T_k_pr_bellow_s6_fin_length_39', header=None)))*293/scaling_factor + - 273
df_2 = trim(np.matrix(pd.read_csv(f'{path}/T_k_pr_above_s6_fin_length_39', header=None)))*293/scaling_factor + - 273
df = df/2+df_2/2

fig = plt.figure(8)
ax = fig.subplots(1,1)
im = ax.imshow(df, 'plasma')
cbar = ax.figure.colorbar(im, orientation='horizontal')
cbar.set_label('Temperature [$^\circ$C]')
ax.axes.xaxis.set_visible(False)
ax.axes.yaxis.set_visible(False)
ax.set_title('Processor with 20, 39mm fins', pad=100)
plt.show()
